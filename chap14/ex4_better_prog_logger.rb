...
def log block_desc, &block
   $nest_depth += 1
   puts "#{"    "*$nest_depth}Beginning block: #{block_desc}"
   returned = block.call
   puts "#{"    "*$nest_depth}#{block_desc.inspect} finished! It returned #{returned}"
   $nest_depth -= 1
end

log "factory opened!" do
    log "box produced!" do
        "box"
    end
    
    log "another box produced!" do
        log "smaller box produced!" do
